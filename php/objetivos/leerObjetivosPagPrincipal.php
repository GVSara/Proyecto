...
if ('GET' === $metodo) {
    ...
    $estado=$datosObjetivos->estado= $_GET["estado"];
    $stmt = $datosObjetivos->leerObjetivoPagPrincipal();
 ...
 if($num>0){
    ...
    echo json_encode(
        array("message" => "No se encontraron objetivos.")
    );
} 

 try{
   $datosObjetivos->leerObjetivoPagPrincipal();
